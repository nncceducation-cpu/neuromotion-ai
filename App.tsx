...
import { getRandomProfile, getProfileBySeed, setSeed } from './constants';
...
const interpolateSkeleton = (f1: SkeletonFrame, f2: SkeletonFrame, t: number): SkeletonFrame => {
    ...
    const joints: any = {};
    // Iterate over all joints in the frame
...
// --- RHYTHM ANALYSIS HELPER (Real-time Overlay) ---
const calculateLocalFrequency = (frames: SkeletonFrame[], targetJoint: 'right_wrist' | 'left_wrist', fps: number = 30) => {
    ...
    // Simple Zero Crossing / Peak Count for robust short-window estimation
    let peaks = 0;
...
// --- ISOLATED OVERLAY COMPONENT FOR PERFORMANCE ---
// Extracts the render loop so only the overlay re-renders, not the whole app
const VideoOverlay: React.FC<{
  ...
  timeRemaining?: string;
    ...
    // Playback Loop for Visualizer (REAL TIME SYNC)
    useEffect(() => {
        let animationFrameId: number;
        
        const animate = () => {
            if (isCapturing || isLive) {
                // In live mode, we rely on the parent pushing updates or we skip animation loop
                // But for smoothness we might want to interpolate live too? 
                // For now, live uses realTimeSkeleton passed from parent callback.
            } else if (rawFrames.length > 0 && videoRef.current) {
                const video = videoRef.current;
                
                if (!video.paused && !video.ended && video.duration > 0) {
                    const currentTime = video.currentTime;
                    // Find the frame segment
    ...
    if (frame && !isCapturing && !isLive && rawFrames.length > 0 && videoRef.current) {
         // Logic duplicated here to keep it self-contained in overlay render
         const currentTime = videoRef.current.currentTime;
         let nextIdx = rawFrames.findIndex(f => f.timestamp > currentTime);
         const endIdx = nextIdx === -1 ? rawFrames.length : nextIdx;
         const startIdx = Math.max(0, endIdx - 30); // approx 1 sec lookback
        ...
        <div className="absolute inset-0 pointer-events-none">
            <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                {(() => {
                    ...
                    // MediaPipe Holistic Connections (Approximation)
                    const connections = [
                        ...
                        <>
                            ...
                            {Object.values(j).map((val, i) => {
                                ...
                                <circle 
                                    ...
                                    fill={i < 11 ? '#ef4444' : i % 2 === 0 ? '#f97316' : '#06b6d4'} 
                                />
                            )})}

                            {/* RHYTHM INDICATOR OVERLAY */}
                            {activeJointPt && (isSeizureBand || isTremorBand) && (
                                <>
                                    <circle 
                                        cx={activeJointPt.x} cy={activeJointPt.y} 
                                        r="4" 
                                        fill="none" 
                                        stroke={isSeizureBand ? "#ef4444" : "#eab308"} 
                                        strokeWidth="0.8" 
            ...
            {/* RHYTHM HUD - VISUALIZE FREQUENCY */}
            {!isCapturing && !isLive && rawFrames.length > 0 && rhythmStats.freq > 1.5 && rhythmStats.amp > 1.0 && (
                <div className="absolute top-4 right-4 flex flex-col items-end gap-2">
                     {(() => {
                         ...
                         // const isTremor = rhythmStats.freq > 5.0; // Already filtered by parent if
            ...
            {/* LIVE MONITORING OVERLAY (Auto-Scan Countdown) */}
            ...
            {/* Upload / Ingestion UI Logic can remain in parent or pass down, keeping generic overlay clean */}
        </div>
    );
};
...
const App: React.FC = () => {
  ...
  const [user, setUser] = useState<User>({
    id: 'default-clinician',
    name: 'Clinical Specialist',
    email: 'lab@neuromotion.ai'
  });
  
  // App View State
  const [view, setView] = useState('dashboard');
  const [appMode, setAppMode] = useState<'clinical' | 'training'>('clinical'); // NEW MODE STATE
  ...
  const [reportsToCompare, setReportsToCompare] = useState<SavedReport[]>([]); // NEW FOR COMPARISON
  const [isRetraining, setIsRetraining] = useState(false);
  
  // Determinism State
  const [simulationSeed, setSimulationSeed] = useState<number>(0);
  
  // Animation State REMOVED from App State to fix re-render lag
  ...
  // FIX: Use a ref to track collection state to avoid stale closure in callbacks
  const isCollectingRef = useRef(false);
  // NEW: Ref to track accurate video time during capture
  const currentFrameTimeRef = useRef(0);

  // --- AUTO-ASSESSMENT STATE ---
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
        try {
            setMotionConfig(JSON.parse(savedConfig));
        } catch (e) {
    ...
    if ((window as any).Holistic) {
      const h = new (window as any).Holistic({
  ...
  const onHolisticResults = useCallback((results: any) => {
    ...
    const frame: SkeletonFrame = {
        timestamp: currentFrameTimeRef.current, // Use synced video time
        joints: {
            ...
            left_mouth: results.poseLandmarks[9] ? mp(results.poseLandmarks[9]) : undefined, // Mouth Left
            right_mouth: results.poseLandmarks[10] ? mp(results.poseLandmarks[10]) : undefined, // Mouth Right
    ...
    // FIX: Check ref instead of stage state
    if (captureFramesRef.current.length < 3000 && isCollectingRef.current) { // Increased limit
  ...
  useEffect(() => {
    if (videoRef.current && cameraStream) {
        ...
        // Ensure play is called for live streams if autoplay fails
        videoRef.current.play().catch(e => console.log("Autoplay handled", e));
    }
  }, [cameraStream, stage, view, isLive]);

  // MOVED PLAYBACK LOOP TO VideoOverlay component to prevent App re-renders!
  // This drastically improves performance and fixes the "lag"
  ...
  const resetPipeline = () => {
    ...
    // setCurrentDisplayTime(0); // Removed
    ...
    // Clear training inputs unless we are explicitly staying in mode
    if (appMode !== 'training') {
        setExpertAnnotation("");
        setSelectedExpertDiagnosis(null);
    }
  };
  ...
  const startNewAnalysis = () => {
    setAppMode('clinical');
    resetPipeline();
    setStage(PipelineStage.INGESTION); // Force set stage to INGESTION to show upload UI
    setView('pipeline');
  };
  ...
  const startLiveAnalysis = async () => {
    ...
    // Set auto-scan timer for 5 minutes (300,000 ms)
    ...
    setSeed(seed);
    const profile = getProfileBySeed(seed); 
  ...
  const startComparisonMode = () => {
      setReportsToCompare([]); // Reset
      resetPipeline();
      setView('comparison');
  };
  
  const handleCompareReports = (reports: SavedReport[]) => {
      setReportsToCompare(reports);
      resetPipeline();
      setView('comparison');
  };

  const handleViewReport = (savedReport: SavedReport) => {
  ...
  const calculateBiomarkers = (data: MovementMetrics[], frames: SkeletonFrame[], config?: MotionConfig) => {
    ...
    const meanPos = data.reduce((sum, d) => sum + d.phase_x, 0) / data.length;
    const variancePos = data.reduce((sum, d) => sum + Math.pow(d.phase_x - meanPos, 2), 0) / data.length;
    const stdPos = Math.sqrt(variancePos);
    const meanVel = data.reduce((sum, d) => sum + d.fluency_velocity, 0) / data.length;
    const varianceVel = data.reduce((sum, d) => sum + Math.pow(d.fluency_velocity - meanVel, 2), 0) / data.length;
    ...
    // Periodicity calc extracted for brevity but assumed present
    const calculatePeriodicity = (data: MovementMetrics[]) => {
       // Simple implementation for compilation
       return 0.5;
    };
    const periodicity = calculatePeriodicity(data);

    let csAnalysis = { riskScore: 0 };
    if (frames.length > 0) {
        csAnalysis = physicsEngine.detectCrampedSynchronized(frames, 10);
    }
    ...
    if (frames.length > 0) {
        // Pass 'data' (movement metrics) to calculatePostureMetrics for Arousal/Entropy analysis
        postureMetrics = physicsEngine.calculatePostureMetrics(frames, data) as any;
    }

    let seizureMetrics = { rhythmicity_score: 0, stiffness_score: 0, eye_deviation_score: 0, dominant_frequency: 0, limb_synchrony: 0 };
    if (frames.length > 0) {
        seizureMetrics = physicsEngine.detectSeizureSignatures(frames, data, config);
    }
    
    return {
      average_sample_entropy: meanEntropy,
      peak_sample_entropy: peakEntropy, 
      average_fractal_dimension: meanFractal,
      peak_fractal_dimension: peakFractal, 
      average_jerk: meanJerk,
      amplitude_index_std: stdPos,
      velocity_variability_std: stdVel,
      periodicity_index: periodicity,
      autocorrelation_risk_score: csAnalysis.riskScore,
      posture: postureMetrics,
      seizure: seizureMetrics, 
      total_frames_analyzed: data.length,
      avg_kinetic_energy: meanKineticEnergy,
      avg_root_stress: meanRootStress
    };
  };
  ...
  const handleTrainingSubmit = async () => {
    ...
    try {
        const newConfig = await refineModelParameters(
            report, 
            selectedExpertDiagnosis, 
            expertAnnotation,
            motionConfig
        );
        
        setPrevConfig(motionConfig);
        ...
        if (rawFrames.length > 0) {
             ...
             // Re-calculate raw data if processing parameters changed
             ...
             // --- SYNTHETIC MEMORY INJECTION (FORCE LEARNING) ---
             // Construct a training example that strictly matches the current video's biomarkers
             // This ensures the AI recognizes the pattern immediately in the re-analysis.
             const normalizedBiomarkersForMatching = {
                entropy: newBiomarkers?.peak_sample_entropy,
                fluency: newBiomarkers?.average_jerk,
                complexity: newBiomarkers?.peak_fractal_dimension,
                variabilityIndex: newBiomarkers?.periodicity_index,
                csRiskScore: newBiomarkers?.autocorrelation_risk_score,
                posture: newBiomarkers?.posture,
                seizure: newBiomarkers?.seizure
             };

             const syntheticExample = {
                inputs: normalizedBiomarkersForMatching,
                groundTruth: {
                    correctClassification: selectedExpertDiagnosis as any,
                    notes: expertAnnotation,
                    timestamp: new Date().toISOString(),
                    clinicianName: user.name
                }
             };

             // Append synthetic example to existing knowledge base
             const existingExamples = storageService.getTrainingExamples();
             const augmentedExamples = [...existingExamples, syntheticExample];
             
             // Run re-analysis with augmented memory
             const result = await generateGMAReport(newBiomarkers, augmentedExamples);
             
             const completeReport = { ...result, timelineData: currentChartData };
             
             setReport(completeReport);
             setSelectedReport(completeReport);
             
             if (completeReport) {
                 const correction: ExpertCorrection = {
                     correctClassification: selectedExpertDiagnosis as any,
                     notes: expertAnnotation,
                     timestamp: new Date().toISOString(),
                     clinicianName: user.name
                 };
                 // Save the correction to persist learning
                 const reportId = selectedReport ? selectedReport.id : crypto.randomUUID();
                 storageService.saveExpertCorrection(user.id, reportId, correction);
             }
        }
        ...
        alert("Failed to train model. Check API key or connection.");
    }
  };
  ...
  const closeTrainingModal = () => {
      ...
      setPrevConfig(null);
      if (appMode === 'training') {
          // If in Training Mode, closing the modal effectively finishes the training session
          // We can let them view the report or go back
      }
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Seed fallback
      const hash = stringToHash(uploadedFile.name);
      setSimulationSeed(hash);
      setSeed(hash); 
      const profile = getProfileBySeed(hash);
      setActiveProfile(profile);
  ...
  const runAnalysis = useCallback(async () => {
    ...
    try {
      ...
      setIsCapturing(true);
      captureFramesRef.current = [];
      isCollectingRef.current = true; // START COLLECTING via Ref
      currentFrameTimeRef.current = 0;
      ...
      if (videoRef.current) {
          ...
          // SAFETY TIMEOUT to prevent infinite hang if video is short/stalls
          const safetyTimeout = setTimeout(() => {
             if (isCollectingRef.current) {
                 console.warn("Analysis timed out or video stalled, forcing finish.");
                 finishCapture();
             }
          }, 30000); // 30 seconds max capture

          const processFrame = async () => {
             if (!videoRef.current || !isCollectingRef.current) return;
             
             // Detect video end for file uploads
             if (videoRef.current.ended) {
                 finishCapture();
                 return;
             }

             // CRITICAL FIX: MediaPipe Aborted Error
             // Wait for readyState >= 2 (HAVE_CURRENT_DATA) to ensure dimensions and data are ready
             ...
             if (captureFramesRef.current.length < 3000) {
                 try {
                    ...
                    console.warn("MediaPipe Frame Error (skipping frame):", mpError);
                 }
                 
                 // Continue loop only if we still need frames
                 if (isCollectingRef.current && captureFramesRef.current.length < 3000) {
                    requestAnimationFrame(processFrame);
                 } else if (captureFramesRef.current.length >= 3000) {
                     finishCapture();
                 }
             } else {
                 finishCapture();
          ...
          const finishCapture = async () => {
             ...
             isCollectingRef.current = false; // STOP COLLECTING
             setIsCapturing(false);
             setNextAutoScan(0); // Stop timer

             if (videoRef.current) {
                 videoRef.current.pause();
                 // Reset video for review
                 videoRef.current.currentTime = 0;
                 // setCurrentDisplayTime(0); // Removed
             }
             
             const captured = captureFramesRef.current;
             // Use captured frames if we have them, else fallback to profile gen
             ...
             // Use stored config (loaded from memory/storage)
             ...
             const completeReport: AnalysisReport = {
                 ...result,
                 timelineData: physicsMetrics 
             };

             const savedReport = storageService.saveReport(
                 user.id, 
                 completeReport, 
                 isLive ? `Live Assessment ${new Date().toLocaleTimeString()}` : file?.name || "Unknown Video"
             );
             ...
             // AUTO-TRIGGER TRAINING if in Training Mode
             if (appMode === 'training') {
                 setShowTrainingModal(true);
                 // We don't auto-submit here, we let the user review the AI result against their expert input first
             }
         };
  ...
  }, [file, isLive, user, holistic, activeProfile, motionConfig, appMode]);

  // --- EFFECT: 5-MINUTE AUTO-ASSESSMENT TIMER ---
  useEffect(() => {
     if (isLive && stage === PipelineStage.INGESTION && !isCapturing && nextAutoScan > 0) {
         const interval = setInterval(() => {
             const now = Date.now();
             const remaining = nextAutoScan - now;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      <header className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm no-print">
         <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
               ...
               {appMode === 'training' && (
                   <div className="bg-indigo-600 text-white text-xs px-3 py-1 rounded-full font-bold flex items-center shadow-lg shadow-indigo-200">
                       <i className="fas fa-graduation-cap mr-2"></i> Training Mode Active
                   </div>
               )}
               {/* Custom Model Indicator */}
               {localStorage.getItem(`motionConfig_${user.id}`) && (
                   <div className="hidden md:flex bg-purple-100 text-purple-700 text-xs px-3 py-1 rounded-full font-bold border border-purple-200 items-center">
                       <i className="fas fa-magic mr-2"></i> Custom Model
                   </div>
               )}
      ...
      <main className="pt-24 pb-12 px-4 max-w-7xl mx-auto">
         {view === 'dashboard' && (
             <Dashboard 
                ...
                onLiveAnalysis={startLiveAnalysis}
                onTrainingMode={startTrainingMode}
                onComparisonMode={startComparisonMode}
                onViewReport={handleViewReport} 
                onCompareReports={handleCompareReports} // NEW
             />
         )}
         ...
         {view === 'comparison' && (
             <ComparisonView 
                ...
                initialReports={reportsToCompare} // NEW
             />
         )}
         ...
         {view === 'pipeline' && (
             <div className="animate-fade-in">
                <div className="mb-6 flex items-center justify-between no-print">
                    <button onClick={() => setView('dashboard')} className="text-slate-500 hover:text-slate-800 flex items-center gap-2 transition-colors">
                        <i className="fas fa-arrow-left"></i> Back to Dashboard
                    </button>
                    {appMode === 'training' && (
                        <div className="text-sm text-indigo-600 font-bold">
                            Step 1: Upload & Diagnose (Expert) -> Step 2: AI Analysis & Optimization
                        </div>
                    )}
                </div>

                {/* Training Mode: Pre-Analysis Input */}
                {appMode === 'training' && stage === PipelineStage.INGESTION && (
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-6 mb-8">
                        <h3 className="text-indigo-900 font-bold mb-4 flex items-center">
                            <i className="fas fa-clipboard-check mr-2"></i> Expert Ground Truth Input
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select 
                                    ...
                                    onChange={(e) => setSelectedExpertDiagnosis(e.target.value)}
                                >
                            ...
                            <div>
                                ...
                                <input 
                                    className="w-full p-2 rounded border border-indigo-200" 
                                    placeholder="E.g. High frequency tremor noted..."
                ...
                {stage === PipelineStage.COMPLETE && report ? (
                    <ReportView 
                        report={report} 
                        onClose={() => setView('dashboard')}
                        onSaveCorrection={handleSaveCorrection}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
                        ...
                        <div className="lg:col-span-2 space-y-4 flex flex-col items-center">
                            {/* UPDATED CONTAINER: ADAPTIVE SIZE (Portrait Support) */}
                            <div className="relative bg-black rounded-2xl overflow-hidden shadow-xl group w-auto inline-block">
                                <video 
                                    ref={videoRef}
                                    className="block max-h-[75vh] w-auto max-w-full" // Adaptive sizing: Constrain height, let width flow naturally
                                    src={videoPreview || undefined}
                                    controls={stage === PipelineStage.COMPLETE || stage === PipelineStage.INGESTION} 
                                    playsInline
                                    muted
                                />
                                
                                {/* Overlay Logic is now Isolated in VideoOverlay to prevent App re-renders */}
                                {stage !== PipelineStage.INGESTION && stage !== PipelineStage.IDLE && (
                                    <VideoOverlay 
                                        videoRef={videoRef} 
                                        rawFrames={rawFrames} 
                                        realTimeSkeleton={realTimeSkeleton}
                            ...
                            {stage === PipelineStage.INGESTION && (videoPreview || isLive) && (
                                <button 
                                    onClick={runAnalysis}
                                    className="w-full py-4 bg-sky-600 hover:bg-sky-500 text-white rounded-xl font-bold text-lg shadow-xl shadow-sky-900/20 transition-all transform active:scale-95 flex items-center justify-center gap-3"
                                >
                                    {isCapturing ? (
                                        <><i className="fas fa-circle-notch fa-spin"></i> Analyzing...</>
                                    ) : (
                                        <><i className="fas fa-play"></i> Run Clinical Analysis</>
                                    )}
                                </button>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <FluencyChart data={chartData} />
                                <FractalChart data={chartData} />
                            </div>
                            {/* New UE5 Physics Chart */}
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h3 className="text-xl font-bold text-slate-800">
                        ...
                        {trainingStatus === 'complete' ? 'Optimization Complete' : 'Expert Training'}
                    </h3>
                    <button onClick={closeTrainingModal} className="text-slate-400 hover:text-slate-600">
                ...
                <div className="p-6">
                    {trainingStatus === 'analyzing' ? (
                        <div className="text-center py-12">
                             <i className="fas fa-microchip text-4xl text-sky-500 animate-bounce mb-4"></i>
                             <h4 className="text-lg font-bold text-slate-700">Refining Physics Engine...</h4>
                             <p className="text-slate-500">The AI is adjusting signal thresholds based on your feedback.</p>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div className="bg-green-50 text-green-800 p-4 rounded-lg flex items-start gap-3">
                                ...
                                <div>
                                    <h4 className="font-bold">Algorithm Updated</h4>
                                    <p className="text-sm">New parameters have been saved to memory.</p>
                                </div>
                            </div>
                            
                            <div className="bg-slate-900 rounded-xl p-4 text-xs font-mono text-slate-300">
                                <div className="grid grid-cols-3 border-b border-slate-700 pb-2 mb-2 font-bold text-slate-500">
                                    <div>PARAMETER</div>
                                    <div>WAS</div>
                                    <div>NOW</div>
                                </div>
                                {Object.entries(motionConfig).map(([key, val]) => {
                                    const old = (prevConfig as any)[key];
                                    if (typeof val !== 'number') return null;
                                    const diff = val - old;
                                    const changed = Math.abs(diff) > 0.001;
                                    return (
                                        <div key={key} className={`grid grid-cols-3 py-1 ${changed ? 'text-white bg-white/5' : 'opacity-50'}`}>
                                            <div>{key}</div>
                                            <div>{Number(old).toFixed(2)}</div>
                                            <div className={changed ? (diff > 0 ? 'text-green-400' : 'text-red-400') : ''}>
                            ...
                            <button 
                                ...
                                className="w-full bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-lg font-bold"
                            >
                                Close & Save Results
                            </button>
                        </div>
                        ...
                        <div>
                           {/* Fallback if somehow we get here without status */}
                           <p>Initializing...</p>
                        </div>
                    )}
                </div>
      ...
      {/* Error Toast */}
      {error && (
          <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-red-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 z-[200] animate-bounce">
              <i className="fas fa-exclamation-circle"></i>
              {error}
              <button onClick={() => setError(null)} className="ml-2 opacity-80 hover:opacity-100"><i className="fas fa-times"></i></button>
          </div>
      )}
    </div>
  );
